clc;clear;
[TrainData,TestData] = Re_data_gene1(0,0,0,0,0);
...
covx_average = (TrainData'*TrainData)./51
...
for i = 1:N
    ...
    sum_cov = sum_cov + covi{i,1};
end
miu_average = sum_miu./N;
cov_average = sum_cov./N
...
%sum = 0;
%for i = 1:N
    %sum = sum + miu{i,1}*miu{i,1}' + 2*covi{i,1}*cov_average^(-1)*covi{i,1};
%end
%S = 1/N*sum -miu_average*miu_average'-2*cov_average;
P1 = zeros(dim,dim);
P2 = zeros(dim,dim);
P3 = zeros(dim,dim);
P4 = zeros(dim,dim);
...
%5.广义特征值分解
[E_vector,E_value] = eig(S,cov_average,'chol');                         %用Cholesky分解，保证广义特征向量正交
for i = 1:dim
...
%6.求解Bn，Bn
Bs_A = E_vector_sort(:,1:n_s)'*cov_average^(-1/2);
E_vector_sort_rev = fliplr(E_vector_sort);
Bn_A = E_vector_sort_rev(:,1:n_ns)'*cov_average^(-1/2);
%我加的
T1 = zeros(Nn+Nt,1);
Bs_w_r = Bs_A;
...
for i = 1:Nn
    ...
    ss1 =Bs_w_r*TrainData(i,:)';        %3*1
    T1(i) = ((ss1-Bs_w_r*miu_average)'*(Bs_w_r*covx_average*Bs_w_r')^(-1)*(ss1-Bs_w_r*miu_average));%^(1/2); 
...
for i = 1:Nt
    ...
    ss1 = Bs_w_r*TestData(i,:)';        %3*1
   % T1(i) = ((ss1-Bs_w*miu_average)'*(Bs_w*cov(TrainData)*Bs_w')^(-1)*(ss1-Bs_w*miu_average))^(1/2); 
    T1(i+Nn) = ((ss1-Bs_w_r*mean(TrainData).')'*(Bs_w_r*cov(TrainData)*Bs_w_r')^(-1)*(ss1-Bs_w_r*mean(TrainData).'));
end
error1 = 0;
for i = 1:time
...
Bs_r = B_r(1:3,:);                        %真实的Bs
Bn_r = flipud(B_r(4:5,:));                        %真实的Bn


%8.求平稳非平稳源
sst =  TestData * Bs_A';             %估计的源
ssn =  TestData * Bn_A';
...
[ err_s, pangles_s ] = subspace_error1(Bs_A', Ps')
[ err_n, pangles_n ] = subspace_error1(Bn_A', Pn')
[ err_a, pangles_a ] = subspace_error1(Bs_A',Bs_r');
